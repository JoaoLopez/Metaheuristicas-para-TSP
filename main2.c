...
int main(int argc, char *argv[]){
    ...
    for(int i = 3; i < argc; i++){
        ...
        //Heurística do Vizinho Mais Próximo Pelos Dois Lados Randomizada
        printf("Solucionando instância pela heurística do vizinho mais próximo pelos dois lados randomizada...\n");
        Tempo_CPU_Sistema(&segCPUInicial, &segSistemaInicial);
        for(int j = 0; j < numeroRepeticoes; j++){
            *statusOperacao = solucionarInstanciaTSPHeuristicaVizinhoMaisProximoDoisLadosRandomizada(instanciaTSP, alpha);
        }
        ...
        if(*statusOperacao == ERRO_GERAR_VETORES_POSSIVEIS_CIDADES_INSERIDAS)   printf("ERRO: ERRO AO GERAR OS VETORES DE POSSÍVEIS CIDADES INSERIDAS!\n");
        ...
        strcat(nomeArquivoSaida, "HVMPDL");
        strcat(nomeArquivoSaida, "_");
